...
export default function LanguageSwitcher() {
  ...
  const [current, setCurrent] = useState<string>(() => {
    try {
      return (
        localStorage.getItem(BLazorCultureKey) ||
        navigator.language ||
        "uz-Latn"
      );
    } catch {
  ...
  return (
    <div
      ref={rootRef}
      className="btn-group position-absolute top-0 end-0 me-5 mt-5"
    >
      <button
        type="button"
        className="btn btn-outline-light btn-lg dropdown-toggle d-flex align-items-center gap-2"
        aria-expanded={open}
        aria-haspopup="true"
        onClick={toggle}
      >
